...
use media_wallet::signer::signer_signature::{
    Secp256k1Signer, Signer, SignerSignature, SignerTrait, SignerType, StarknetSignature, StarknetSigner,
...
fn change_owner() {
    ...
    // MediaWallet events
    let signer_link_event = MediaWallet::Event::SignerLinked(
        SignerLinked { signer_guid: new_owner_guid, signer: signer },
    );
    let other_signer_link_event = MediaWallet::Event::SignerLinked(
...
#[test]
fn get_name() {
    assert_eq!(initialize_account().get_name(), 'MediaWallet');
}

#[test]
fn getName() {
    assert_eq!(initialize_account().getName(), 'MediaWallet');
}
