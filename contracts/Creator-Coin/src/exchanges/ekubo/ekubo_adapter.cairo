...
use creator_coin::utils::sort_tokens;
...
impl EkuboAdapterImpl of creator_coin::exchanges::ExchangeAdapter<
...
> {
    fn create_and_add_liquidity(
        ...
        let creator_coin = ICreatorCoinDispatcher { contract_address: token_address, };
        let this = get_contract_address();
        creator_coin.transfer(ekubo_launchpad.contract_address, creator_coin.balance_of(this));
        ...
        let total_supply = creator_coin.total_supply();
        let max_returned_tokens = PercentageMath::percent_mul(total_supply, 50);
        assert(creator_coin.balanceOf(this) < max_returned_tokens, 'ekubo has returned tokens');
        ...
        assert(creator_coin.balanceOf(this) >= team_allocation, 'failed buying team tokens');
...
fn buy_tokens_from_pool(
    ...
    let token_to_buy = ICreatorCoinDispatcher { contract_address: token_to_buy };
