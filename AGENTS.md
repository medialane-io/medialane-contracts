...
| `nftcomments-deployer` | see AGENTS.md NFTComments section | NFTComments upgrade deployer |
...
### Medialane-Protocol (`contracts/Medialane-Protocol-ERC721/`)

Core ERC721 marketplace contract (order registration, fulfillment, cancellation). Audited, hardened, and redeployed 2026-04-26.

- **Contract address**: `0x00f8ccaae0bc811c79605974cc1dab769b9cea8877f033f8e3c17f30457caba6`
- **Class hash**: `0x03dff4f34b976207246207954263be9a28b51390321702443291088dcdf4b2e6`
- **Declare tx**: `0x052d5efd6ae351630adbcc57a37ecf429d58072640be96f9b6e50b29c76ca551`
- **Deploy tx**: `0x00b7bf96fe2a9c533c16c1516fcc30820369afdd05b9933ed8c2558e6034bf67`
- **Deployer account**: `medialane-deployer` (`0x06acfcef048dcaac4a11fab313507d53145ed2a468f2a6188527918f1b12d935`)
- **Native token**: STRK (`0x04718f5a0fc34cc1af16a1cdee98ffb20c31f5cd61d6ab07201858f4287c938d`)
- **snfoundry.toml profiles**: `medialane-mainnet`, `medialane-deployer` (in `contracts/Medialane-Protocol-ERC721/snfoundry.toml`)

**Previous deployment**: `0x004387e58d469f19332dd5d20846b10339ddc49ef208025ec7d5bef294a8daf3` with class hash `0x079225381275ff71a723f560419bbe4e69dad324622226fc6593298577b824bc` (superseded)

**Key hardening in this deployment (2026-04-26):**
- Registration allowed any time before expiry; fulfillment remains gated by the active window
- Invalid `start_time` / `end_time` windows rejected before storage
- Registration-time validation for token addresses, consideration recipient, and canonical item fields
- `NATIVE` items require zero `token` and zero `identifier_or_criteria`
- `ERC20` items require zero `identifier_or_criteria`
- `fulfill_order` rejects zero `fulfiller`
- Caller-bound fulfillment, offerer-bound cancellation, fixed-price enforcement, and CEI preserved
...
PATH="..." sncast --profile medialane-deployer declare --contract-name Medialane
PATH="..." sncast --profile medialane-deployer deploy \
  --class-hash <new_class_hash> \
  --arguments '<native_token_address>'
```
...
ERC-1155 marketplace v2 with ERC721-style order architecture, partial fills, and ERC-2981 royalties. Audited, hardened, and redeployed 2026-04-26.
...
- **Deployer account**: `medialane-deployer` (`0x06acfcef048dcaac4a11fab313507d53145ed2a468f2a6188527918f1b12d935`)
- **Native token**: STRK (`0x04718f5a0fc34cc1af16a1cdee98ffb20c31f5cd61d6ab07201858f4287c938d`)
- **snfoundry.toml profile**: `medialane-deployer`

**Previous deployments**:
- `0x035836932ba1d219e00b8e42cd9a433fb2b211a08edcaa8bae40232f335f777d` (legacy ERC1155 architecture)
- `0x03aab04e806542cd88bfd0c5bb2a37334fd742d477a2e0f97af09aa4a36137ca` (2026-04-20 partial-fill redesign; superseded by v2)
- `0x042005e9b85536072bfa260b95aa6aaef07f48e622031657384d2375195d7123` (broken — never emitted any OrderCreated events)

**Key changes in this deployment (2026-04-26):**
- ERC721-style shared vocabulary: `ItemType`, `OfferItem`, `ConsiderationItem`, `OrderParameters`
- Contract surface renamed to `Medialane1155V2` / `IMedialane1155V2`
- Immutable deployment: no owner, no admin role, no upgrade hook
- Partial fills: buyer chooses `quantity` (1 ≤ Q ≤ remaining_amount); order stays `Created` until fully consumed
- ERC-2981 royalty overlay; primary sale recipient comes from `consideration.recipient`
- Launch trade shapes: ERC1155 offer for ERC20 or NATIVE consideration
- `OrderFulfilled` event includes `quantity`, `remaining_amount`, `sale_amount`, and royalty fields

**Build + declare + deploy workflow:**
```bash
cd contracts/Medialane-Protocol-ERC1155
SCARB_CACHE=/tmp/scarb-cache-erc1155-v2 ~/.asdf/shims/scarb build

SCARB_CACHE=/tmp/scarb-cache-erc1155-v2 ~/.asdf/shims/sncast --profile medialane-deployer --wait declare --contract-name Medialane1155V2
~/.asdf/shims/sncast --profile medialane-deployer --wait deploy \
  --class-hash <new_class_hash> \
