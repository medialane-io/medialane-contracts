...
use creator_coin::tests::addresses::{JEDI_FACTORY_ADDRESS, JEDI_ROUTER_ADDRESS, ETH_ADDRESS};
...
fn test_jediswap_integration() {
    ...
    let first_swap = router
        .swap_exact_tokens_for_tokens(
            amountIn: amount_in,
            amountOutMin: 0,
            path: array![quote_address, creator_coin_address],
            to: owner,
    ...
    creator_coin.approve(JEDI_ROUTER_ADDRESS(), first_out);
    ...
    let _second_swap = router
        .swap_exact_tokens_for_tokens(
            amountIn: first_out,
            amountOutMin: 0,
            path: array![creator_coin_address, quote_address],
            to: owner,
            deadline: starknet::get_block_timestamp()
        );
